...
export const HoverEffect = ({
  ...
  return (
    ...
    >
      {items.map((item, idx) => (
        <Link
          href={`products/${item?.id.toString()}`}
          key={item?.title}
        ...
        >
          <AnimatePresence>
            {hoveredIndex === idx && (
              <motion.span
                className="absolute inset-0 block h-full w-full rounded-3xl bg-green-900  dark:bg-slate-800/[0.8]"
          ...
          <Card>
            ...
            <CardDescription>
              <Image src={''} alt={'muah'} width={1080} height={900} />
            </CardDescription>
          </Card>
        </Link>
      ))}
